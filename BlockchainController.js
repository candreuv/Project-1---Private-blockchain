...
class BlockchainController {
    ...
    // This endpoint allows you to validate the blockchain integrity
    validateChain() {
        this.app.get("/validateChain", async (req, res) => {
            try {
                let errorLog = await this.blockchain.validateChain();
                if(errorLog.length > 0){
                    return res.status(500).json("Invalid blockchain!");
                } else {
                    return res.status(200).send("Valid blockchain");
                }
            } catch (error) {
                return res.status(500).send("An error happened during validation!");
            }
        });
    }
